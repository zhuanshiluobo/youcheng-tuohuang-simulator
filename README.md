...
- 雷蛇、极境、德克萨斯、坎诺特、锡人五张标准角色牌的盖放、策略/计谋、待选结算和收尾回收；独立底部手牌支持阶段化扇形、悬停、本机本局排序、拖动盖放及手牌/弃牌双区预览。
...
- `docs/模块文档/底部手牌与弃牌预览模块说明.md`：底部手牌、排序、盖放拖动、弃牌模态窗口和状态权威边界。
- `docs/工作记录/已完成模块工作记录/2026-08-15_底部手牌与弃牌预览实现记录.md`：角色手牌 UI 的实现范围与验证证据。
