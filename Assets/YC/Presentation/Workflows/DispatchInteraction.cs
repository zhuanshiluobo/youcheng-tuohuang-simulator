...
{
    /// <summary>把影响力 Presenter 的调度阶段包装为独立交互。</summary>
    public sealed class DispatchInteraction : InteractionBase
    {
        private readonly InfluenceActionPresenter presenter;

        public DispatchInteraction(InfluenceActionPresenter presenter)
        {
            this.presenter = presenter ?? throw new ArgumentNullException(nameof(presenter));
        }

        public override string Id => "active.dispatch";

        public override InteractionPriority Priority => InteractionPriority.ActiveAction;

        public override bool IsActive =>
            presenter.IsSelectingDispatchSource ||
            presenter.IsSelectingDispatchTarget ||
            presenter.IsChoosingDispatchContinuation;
